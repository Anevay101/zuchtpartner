...
const BOOLEAN_FIELDS = ['disease_free', 'breeding_allowed', 'learning_file', 'in_breeding_station', 'flaxen_carrier'];
const JSONB_KEYS = [
...
function visibleFlaxenFromPhenotype(data) {
  if (typeof inferGeneticHintsFromPhenotype !== 'function') return false;
  return [data?.coat_color, data?.notes, data?.name].some((text) =>
    inferGeneticHintsFromPhenotype(text).some((h) => h?.locus === 'Flaxen' && String(h?.allele || '').toLowerCase() === 'flfl')
  );
}

// Explizites Stammdaten-Feld und die bestehende Farbgenetik arbeiten
// zusammen: "Ja" dokumentiert mindestens eine fl-Kopie; bei sichtbar
// flaxenem Pferd bleibt der stärkere Zustand flfl erhalten. "Nein" setzt
// den Locus bewusst auf nicht vorhanden. Alte Datensätze ohne eigenes
// Feld behalten ihren bisherigen Override/ihre Phänotyp-Ableitung.
function syncFlaxenCarrierRecord(payload) {
  if (!payload || typeof payload !== 'object') return payload;
  if (payload.flaxen_carrier == null) return payload;

  const overrides = { ...(payload.color_gene_overrides || {}) };
  const visible = visibleFlaxenFromPhenotype(payload);

  if (payload.flaxen_carrier === true) {
    overrides.Flaxen = visible ? 'hom' : 'het';
  } else if (visible) {
    // Sichtbares Flaxen beweist flfl; eine versehentliche "Nein"-Auswahl
    // darf diese sichere Information nicht zerstören.
    payload.flaxen_carrier = true;
    overrides.Flaxen = 'hom';
  } else {
    overrides.Flaxen = 'absent';
  }
  payload.color_gene_overrides = overrides;
  return payload;
}

function derivedFlaxenCarrierValue(data) {
  if (visibleFlaxenFromPhenotype(data)) return true;
...
function updateAppaloosaPatternVisibility() {
  ...
  // V54.0.16: Die Appaloosa-Abfrage sitzt direkt zwischen den Stammdaten.
  // Kein separates Modal und keine Auswahlbuttons am Seitenende mehr.
  // Sobald Appaloosa/LP relevant ist, erscheint genau ein normales
  // Auswahlfeld mit allen Musteroptionen.
  field.hidden = !state.relevant;
...
function maybePromptAppaloosaPattern() {
  ...
  activateTab('stammdaten');
  field.hidden = false;
  field.classList.add('appaloosa-pattern-attention');
...
async function runSaveFlow() {
  ...
  syncFlaxenCarrierRecord(payload);
...
// Detaildarstellung, Tabs, Genetik- und Stammbaum-HTML stehen seit V54.0.75 in horse-detail-render.js.
