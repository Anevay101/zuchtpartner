...
(() => {
  ...
  const EN = {
    ...
    'Heu':'Hay', 'Stroh':'Straw', 'Ballen':'bales', 'Säcke':'bags', 'Einheiten':'units', 'Pferdetage':'horse-days', 'Heute fällig':'Due now',
    ...
    '„Bedarf“ zeigt nur den Verbrauch im gewählten Zeitraum. Gekauft wird ausschließlich die Spalte „Kaufen“: 1 Sack Kraftfutter = 30 Einheiten, 1 Ballen Heu/Stroh = 30 Pferdetage. Angebrochene Vorräte werden rechnerisch zum nächsten Bestelltermin mitgenommen.':'“Requirement” only shows consumption during the selected interval. Only the “Buy” column is the actual order: 1 bag of concentrated feed = 30 units, 1 bale of hay/straw = 30 horse-days. Calculated leftovers are carried over to the next due date.',
    ...
    '📈 Zuchtschau-Entwicklung':'📈 Breeding show trend', '📈 Zuchtschau-Entwicklung:':'📈 Breeding show trend:', '📈 Aktuelles Schau-Niveau':'📈 Current show level',
    ...
    'Schauen':'shows', 'Meldungen':'entries', 'besetzte Schauen':'completed shows', 'eingelesen':'imported', 'gemeinsam gespeichert':'saved for everyone',
    ...
    'Aktuelles Schau-Niveau aus eingelesenen MDR-Ergebnislisten. Stuten und Hengste werden getrennt ausgewertet.':'Current show level from imported MDR result lists. Mares and stallions are analysed separately.',
    'Die App erkennt Rasse sowie Körung/Prämierung bzw. Stallions/Mares automatisch. Gemeinsam gespeichert werden nur Datum und Punktelisten je Schautag – keine Pferdenamen, Besitzer oder kopierten Seitentexte.':'The app automatically detects breed and stallion/mare show type. Only dates and point lists per show day are shared; horse names, owners and pasted page text are not stored.',
  ...
  const ATTR_EN = {
    ...
    'Geschlecht für Zuchtschau-Entwicklung':'Sex for breeding-show trend', 'Geschlecht für aktuelles Schau-Niveau':'Sex for current show level',
    'Hier den kopierten Seitentext einfügen…':'Paste the copied page text here…',
    'Speichern & nächstes Pferd (alphabetisch)':'Save & next horse (alphabetically)',
  ...
  Object.assign(EN, {
    'Pxx / Einordnung':'Pxx / assessment', 'Pxx':'Pxx', 'Empfehlung':'Recommendation', 'Empfehlung / Einordnung':'Recommendation / assessment', 'Empfehlung & INT':'Recommendation & INT', 'Empf.':'Rec.', 'Ampel':'Traffic light',
    ...
    'keine auffällige Nebenbegabung':'no notable secondary talent', 'keine auffällige ab P45':'none notable from P45', 'keine auffällige ab 45/100':'none notable from 45/100',
    'Nur P45+ wird hervorgehoben.':'Only P45+ is highlighted.', 'Nur Empfehlungen ab 45/100 werden hervorgehoben.':'Only recommendations at 45/100 or above are highlighted.',
    'Empfehlungen kopieren':'Copy recommendations',
    'Keine Nebenbegabung ab P45 erkannt.':'No secondary talent at P45 or above detected.', 'Keine Nebenbegabung mit Empfehlung ab 45/100 erkannt.':'No secondary talent with a recommendation of 45/100 or above detected.',
    'Keine Nebenbegabung ab P45 in der aktuellen Auswahl.':'No secondary talent at P45 or above in the current selection.', 'Keine Nebenbegabung mit Empfehlung ab 45/100 in der aktuellen Auswahl.':'No secondary talent with a recommendation of 45/100 or above in the current selection.', 'Keine belastbare Turniereinordnung':'No reliable competition assessment',
    ...
    'Wie sinnvoll ist die Disziplin für Turniere? Der Empfehlungswert 0–100 kombiniert die relative Stärke innerhalb derselben Disziplin + LK mit einer realistischen Punktekurve. Nebenbegabungen ab 45/100 werden separat hervorgehoben.':'How worthwhile is this discipline for competitions? The 0–100 recommendation score combines relative strength within the same discipline + level with a realistic points curve. Secondary talents from 45/100 are highlighted separately.',
    'Welche Pferde sind in dieser Disziplin wirklich turnierrelevant? Die Rangliste sortiert LK-übergreifend nach dem Empfehlungswert; Rohpunkte und INT bleiben sichtbar.':'Which horses are genuinely competition-relevant in this discipline? Across levels, the ranking is sorted by the recommendation score; raw points and INT remain visible.',
    'Der Empfehlungswert 0–100 kombiniert Pxx mit einer weichen Punktekurve: LK10 ab 155, LK9 ab 190, LK8 ab 200 Punkten. Pxx vergleicht primär exakt dieselbe Disziplin + LK; bei weniger als 15 Vergleichswerten wird auf Hauptgruppe + LK und danach auf die gesamte LK zurückgefallen. INT wird separat bewertet.':'The 0–100 recommendation score combines Pxx with a smooth points curve: LK10 from 155, LK9 from 190, LK8 from 200 points. Pxx primarily compares the exact same discipline + level; with fewer than 15 comparison values it falls back to main group + level and then the whole level. INT is assessed separately.',
    'Der Empfehlungswert 0–100 kombiniert die relative Pxx-Stärke mit einer weichen realistischen Turnierkurve: LK10 ab 155, LK9 ab 190, LK8 ab 200 Punkten. Pxx wird weiterhin aus derselben Disziplin + LK berechnet und bei kleiner Stichprobe auf Gruppe+LK bzw. LK gesamt zurückgeführt. INT bleibt separat: ≤2,00 sehr gut, 2,01–2,50 gut machbar, >2,50 mühsamer.':'The 0–100 recommendation score combines relative Pxx strength with a smooth realistic competition curve: LK10 from 155, LK9 from 190, LK8 from 200 points. Pxx is still calculated from the same discipline + level and falls back to group + level or the whole level for small samples. INT remains separate: ≤2.00 very good, 2.01–2.50 manageable, >2.50 more laborious.',
    'Der sichtbare Empfehlungswert 0–100 kombiniert die relative Pxx-Stärke mit einer weichen realistischen Turnierkurve: LK10 ab 155, LK9 ab 190, LK8 ab 200 Punkten. Pxx bleibt intern die Vergleichsbasis aus derselben Disziplin + LK; bei kleiner Stichprobe wird auf Gruppe+LK bzw. LK gesamt zurückgefallen. INT bleibt separat: ≤2,00 sehr gut, 2,01–2,50 gut machbar, >2,50 mühsamer.':'The visible 0–100 recommendation score combines relative Pxx strength with a smooth realistic competition curve: LK10 from 155, LK9 from 190, LK8 from 200 points. Pxx remains the internal comparison basis from the same discipline + level; for small samples it falls back to group + level or the whole level. INT remains separate: ≤2.00 very good, 2.01–2.50 manageable, >2.50 more laborious.',
    'Angezeigt werden nur Nebenbegabungen ab 45/100: 80+ sehr interessant · 65–79 interessant · 45–64 situativ.':'Only secondary talents at 45/100 or above are shown: 80+ highly worthwhile · 65–79 worthwhile · 45–64 situational.',
  ...
  const DYNAMIC_EN = [
    ...
    [/^(\d+) Pferde · (\d+) auswertbare Disziplin\/LK-Werte · interne Pxx-Basis primär aus exakt gleicher Disziplin \+ LK\. Berechnung vollständig lokal aus dem bereits synchronisierten Bestand\.$/s, '$1 horses · $2 evaluable discipline/level values · internal Pxx basis primarily from the exact same discipline + level. Calculated entirely locally from the already synchronised stock.'],
