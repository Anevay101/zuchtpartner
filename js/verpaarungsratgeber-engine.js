...
function exteriorComplementarityScore(mare, stallion) {
  const mareRows = mare?.exterior_genetics?.rows || [];
  const stallionByLabel = new Map((stallion?.exterior_genetics?.rows || []).map((r) => [r.label, r]));
  let fixedTotal = 0, fixed = 0, heldTotal = 0, held = 0, secured = 0;
  for (const mareRow of mareRows) {
    const stallionRow = stallionByLabel.get(mareRow.label);
    if (!stallionRow) continue;
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (i < 4) {
        if (zA === 0) {
          fixedTotal++;
          if (zB >= 1) { fixed++; if (zB === 2) secured++; }
        } else {
          heldTotal++;
          if (zB >= 1) { held++; if (zB === 2) secured++; }
        }
      } else {
        if (zA >= 1) {
          fixedTotal++;
          if (zA === 1 && zB <= 1) { fixed++; if (zB === 0) secured++; }
        } else {
          heldTotal++;
          if (zB <= 1) { held++; if (zB === 0) secured++; }
        }
      }
    }
  }
  return { atStake:fixedTotal, saved:fixed, fixedTotal, fixed, heldTotal, held, secured };
}
...
function extComplementarityScore(mare, stallion) {
  ...
  for (const mareRow of mareRows) {
    ...
    const stallionTokens = parseExteriorTokens(stallionRow.genotype);
    if (!mareTokens || !stallionTokens) continue;
    const isWeakTrait = ownExteriorTraitScore(mareTokens) >= 3;
    for (let i = 0; i < 8; i++) {
      const zA = parseExteriorLocus(mareTokens[i]);
      const zB = parseExteriorLocus(stallionTokens[i]);
      if (isWeakTrait) {
        if (i < 4 && zA === 0) {
          fixedTotal++; if (zB >= 1) { fixed++; if (zB === 2) secured++; }
        } else if (i >= 4 && zA >= 1) {
          fixedTotal++; if (zA === 1 && zB <= 1) { fixed++; if (zB === 0) secured++; }
        }
      } else {
        if (i < 4 && zA >= 1) {
          heldTotal++; if (zB >= 1) { held++; if (zB === 2) secured++; }
        } else if (i >= 4 && zA === 0) {
          heldTotal++; if (zB <= 1) { held++; if (zB === 0) secured++; }
        }
      }
    }
  }
  return {
    atStake:fixedTotal, saved:fixed, fixedTotal, fixed, heldTotal, held, secured,
    weighted: fixed * 1000000 + held * 1000 + secured,
  };
}
...
function gpComplementarityScore(mare, stallion) {
  const mareTraitMap = flattenTraitPotentials(mare?.traits);
  const stallionTraitMap = flattenTraitPotentials(stallion?.traits);
  let fixedTotal = 0, fixed = 0, heldTotal = 0, held = 0;
  const countGroup = (names, mapA, mapB) => {
    ...
    for (const name of names) {
      ...
      if (a < avg) { fixedTotal++; if (b > a) fixed++; }
      else { heldTotal++; if (b >= a) held++; }
    }
  };
  ...
  return { atStake:fixedTotal, saved:fixed, fixedTotal, fixed, heldTotal, held, secured:0 };
}
...
function computeEmpiricalDeviations(allHorses) {
  ...
  for (const child of cleanHorses) {
    ...
    const fatherName = anc[0] && !isUnknownAncestorName(anc[0]) ? anc[0] : null;
    const motherName = anc[1] && !isUnknownAncestorName(anc[1]) ? anc[1] : null;
  ...
  for (const metric of Object.keys(EMPIRICAL_METRICS)) {
    ...
    const meanDiff = values.length
      ? values.reduce((a, b) => a + b, 0) / values.length
      : null;

    // V51: echte Streuung um die Datenbank-Schätzung.
    // Datenbank-Schätzung = Elternmittel + meanDiff.
    // Daher ist (echte Abweichung - meanDiff) der Fehler der Schätzung.
    // Die 10./90.-Perzentile bilden den zentralen 80%-Bereich der bisher
    // beobachteten Fohlen – kein theoretischer Best/Worst-Case.
    const residuals = meanDiff == null
      ? []
      : values.map(v => v - meanDiff).sort((a,b) => a-b);

    result[metric] = {
      n: values.length,
      meanDiff,
      typicalCoverage: residuals.length ? 0.80 : null,
      typicalLowOffset: residuals.length ? quantile(residuals, 0.10) : null,
      typicalHighOffset: residuals.length ? quantile(residuals, 0.90) : null,
    };
  }
  return result;
}
...
function colorWishPossible(candidate, wish, flaxenLookup, flaxenChildrenByName) {
  ...
  // Sichtbares, manuell bestätigtes Appaloosa-Muster zählt als
  // Phänotyp-Nachweis für mindestens ein Lp, auch wenn der Gentest fehlt.
  if (
    wish.locus === 'Appaloosa' &&
    candidate?.appaloosa_pattern &&
    candidate.appaloosa_pattern !== 'anderes / unklar'
  ) {
    return true;
  }
...
function complementPercent(c) {
  if (!c) return 100;
  const total = c.fixedTotal ?? c.atStake;
  const fixed = c.fixed ?? c.saved;
  if (!total) return 100;
  return (fixed / total) * 100;
}

function compareComplementCandidates(a, b, schwerpunkt) {
  const ca = a?.complement, cb = b?.complement;
  if (!ca && !cb) return 0;
  if (!ca) return 1;
  if (!cb) return -1;
  if (schwerpunkt === 'int') {
    const wa = ca.weighted ?? ca.saved ?? 0;
    const wb = cb.weighted ?? cb.saved ?? 0;
    return wb - wa;
  }
  const fields = ['fixed', 'held'];
  if (schwerpunkt === 'ext' || schwerpunkt === 'extpct') fields.push('secured');
  for (const field of fields) {
    const av = ca[field] ?? (field === 'fixed' ? ca.saved : 0) ?? 0;
    const bv = cb[field] ?? (field === 'fixed' ? cb.saved : 0) ?? 0;
    if (av !== bv) return bv - av;
  }
  const pa = complementPercent(ca), pb = complementPercent(cb);
...
function rankStallions(mare, stallions, {
  ...
  const candidates = stallions.filter((stallion) => {
    if (findSharedNames(mare, stallion).length > 0) {
      exclusionStats.related += 1;
      return false;
    }
    if (mareHasOvero && hasOveroGene(stallion)) {
      exclusionStats.overo += 1;
      return false;
    }
    if (wishes.length && !wishes.every((wish) => colorWishPossible(stallion, wish, flaxenLookup, flaxenChildrenByName))) {
      exclusionStats.colorWish += 1;
      return false;
    }
    return true;
  });
  ...
  scored.sort((a, b) => {
    if (mode === 'complement') {
      const cmp = compareComplementCandidates(a, b, schwerpunkt);
      if (cmp) return cmp;
    }
  ...
  return { total: stallions.length, candidateCount: candidates.length, all: scored, top: scored.slice(0, RANK_RESULT_COUNT), exclusionStats };
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    COLOR_WISH_OPTIONS, colorWishPossible, rankStallions, compareComplementCandidates, complementPercent,
