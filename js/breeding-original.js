...
function isBreedNameInsteadOfAncestor(name, horseA, horseB) {
  const n = normalizeName(name);
  if (!n) return false;
  const breeds = [horseA?.breed, horseB?.breed]
    .filter(Boolean)
    .map(v => normalizeName(typeof normalizeBreed === 'function' ? normalizeBreed(v) : v));
  return breeds.includes(n);
}
...
function findSharedNames(mare, stallion) {
  const pool = foalPedigreeNodes(mare, stallion)
    ...
    .filter((entry) => !isUnknownAncestorName(entry.name))
    .filter((entry) => !isBreedNameInsteadOfAncestor(entry.name, mare, stallion));
  ...
  for (const entries of byNormalized.values()) {
    // WICHTIG:
    // Eine Dopplung ist für DIESE Verpaarung nur dann ein gemeinsamer
    // Vorfahr, wenn der Name auf BEIDEN Elternseiten vorkommt.
    //
    // Eine Wiederholung nur innerhalb der Mutterseite oder nur innerhalb
    // der Vaterseite darf die Paarung nicht komplett ausschließen.
    // Das ist besonders für EN-Pedigrees wichtig, weil der kopierte
    // Stammbaum je nach Darstellung den Namen des Pferdes selbst noch
    // einmal innerhalb seiner eigenen Seite enthalten kann.
    const sides = new Set(entries.map(e => e.side));
    if (!(sides.has('Mutter') && sides.has('Vater'))) continue;
...
function pedigreeNamePool(horse) {
  ...
  const ownBreed = normalizeName(
    typeof normalizeBreed === 'function' ? normalizeBreed(horse?.breed) : horse?.breed
  );
  pedigreeAncestorNames(horse).forEach((name, i) => {
    if (isUnknownAncestorName(name)) return;
    if (ownBreed && normalizeName(name) === ownBreed) return;
